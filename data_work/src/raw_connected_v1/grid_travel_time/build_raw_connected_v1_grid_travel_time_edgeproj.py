...
def parse_args():
    ...
    parser.add_argument(
        "--output-root",
        default=str(Path(str(DEFAULT_OUTPUT_ROOT)).with_name("raw_connected_v1_grid_travel_time_edgeproj_v1")),
    )
    parser.add_argument(
        "--speed-cache-dir",
...
def build_edge_projection_connectors(
    ...
    for grid_row in grid_3857.itertuples(index=False):
        ...
        for rank, row in enumerate(chosen.itertuples(index=False), start=1):
            ...
            connector_ff_time = 60.0 * (float(row.connector_dist_m) / 1000.0) / float(row.speed_ff_kmh)
            records.append(
                {
                    ...
                    "connector_ff_time_min": connector_ff_time,
                    ...
                    "host_edge_speed_ff_kmh": float(row.speed_ff_kmh),
                }
            )
...
def compute_grid_costs_adjacent_pairwise(
    ...
    edge_ff_time_lookup = road_edges.set_index("edge_id")["tau_ff_min"].to_dict()
    ...
    connectors["host_edge_ff_time_min"] = connectors["host_edge_id"].map(edge_ff_time_lookup).fillna(0.0)
    ...
    connectors["outbound_edge_ff_time"] = connectors["host_edge_ff_time_min"] * (1.0 - connectors["access_fraction"].clip(0.0, 1.0))
    connectors["inbound_edge_ff_time"] = connectors["host_edge_ff_time_min"] * connectors["access_fraction"].clip(0.0, 1.0)
    ...
    road_ff_wts = road_edges["tau_ff_min"].to_numpy(dtype=float)
    ...
    for pair in adjacent_pairs.itertuples(index=False):
        ...
        if home not in conn_by_grid or work not in conn_by_grid:
            out_rows.append({
                ...
                "raw_connected_tau_ff_min": np.nan,
                "tau_obs_min": np.nan,
                "tau_ff_min": np.nan,
                ...
                "lane_resolved_for_bpr": np.nan,
                "xi_bpr_edgeproj": np.nan,
            })
            continue
        ...
        cff_h = (home_conn["connector_ff_time_min"] + home_conn["outbound_edge_ff_time"]).to_numpy(dtype=float)
        ...
        cff_w = (work_conn["inbound_edge_ff_time"] + work_conn["connector_ff_time_min"]).to_numpy(dtype=float)
        ...
        all_ff_wts = np.concatenate([road_ff_wts, cff_h, cff_w])
        ...
        if np.isfinite(tt):
            ...
            if cur == HOME_IDX:
                ...
                route_ff_min = float(sum(ff_time_dict.get(k, 0.0) for k in arc_keys))
                lane_weight_sum = 0.0
                lane_len_sum = 0.0
        ...
        lane_resolved = lane_est_path_weighted
        if not (np.isfinite(lane_resolved) and lane_resolved > 0):
            lane_resolved = lane_est_path_bottleneck
        if not (np.isfinite(lane_resolved) and lane_resolved > 0):
            lane_resolved = LANES_FALLBACK
        congestion_ratio = (
            max(tt, route_ff_min) / route_ff_min
            if np.isfinite(tt) and np.isfinite(route_ff_min) and route_ff_min > 0
            else np.nan
        )
        xi_bpr = (
            float(lane_resolved) * (congestion_ratio ** (1.0 / DELTA1))
            if np.isfinite(congestion_ratio) and congestion_ratio > 0
            else np.nan
        )
        avg_speed = (
        ...
        out_rows.append({
            ...
            "raw_connected_tau_ff_min": route_ff_min,
            "tau_obs_min": tt,
            "tau_ff_min": route_ff_min,
            ...
            "lane_resolved_for_bpr": lane_resolved,
            "xi_bpr_edgeproj": xi_bpr,
        })
...
def main():
    ...
    config_payload = {
        ...
        "path_tau_obs_source": f"raw_connected_v1 edgeproj path using {speed_stats.get('period_speed_source')}",
        "path_tau_ff_source": "same realized edgeproj path using speed_ff_kmh from 22:00-05:00 speed table; missing free-flow speed falls back to period speed",
        "xi_bpr_formula": "lane_resolved_for_bpr * max(tau_obs/tau_ff, 1)^(1/delta1)",
        "delta1": DELTA1,
        "lanes_fallback": LANES_FALLBACK,
    }
    config_text = json.dumps(config_payload, indent=2, ensure_ascii=False)
    (output_root / f"run_config_{args.period}.json").write_text(config_text, encoding="utf-8")
    (output_root / "run_config.json").write_text(config_text, encoding="utf-8")
