...
remainder = readmatrix(remainder_data);
sparse_traffic = sparse(bilateral(:,1), bilateral(:,2), bilateral(:,3), 217, 217);
sparse_t_bar_hat_asym = sparse(remainder(:,1), remainder(:,2), .99, 217, 217);
l_R = nodes(:,2);
l_F = nodes(:,3);
N = length(l_R);
...
parfor i = 1:m
    ...
    chi_lr_lf_export_name = strcat('chi_lr_lf_ber_nc_', num2str(index(1)), '_', num2str(index(2)), '.csv');
    traffic_export_name = strcat('traffic_ber_nc_', num2str(index(1)), '_', num2str(index(2)), '.csv');
    t_hat_export_name = strcat('t_hat_ber_nc_', num2str(index(1)), '_', num2str(index(2)), '.csv');
    err_export_name = strcat('err_ber_nc_', num2str(index(1)), '_', num2str(index(2)), '.csv');
