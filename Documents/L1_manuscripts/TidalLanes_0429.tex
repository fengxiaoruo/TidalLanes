...
Figure~\ref{fig:pipeline-overview} summarizes the end-to-end data-processing workflow used in this paper. We start from raw segment-level speed records and the baseline road network. We then construct a directed-centerline system to characterize tidal patterns, quantify directional asymmetries in speed and flow, and identify potential tidal-lane candidates for counterfactual analysis.\\
 ...
 In parallel, we build a grid system and compute grid-level travel-cost matrices directly from the raw network. To recover connectivity, we develop a road-connection algorithm with manual corrections in the topology. We then solve shortest paths and obtain origin--destination travel cost, distance, and implied speed measures on the raw road network.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\begin{table}[h!]
		...
		\begin{minipage}{\textwidth}
			\footnotesize
			\raggedright
			Note: coverage is defined relative to the retained baseline directed centerline network.
		\end{minipage}
	\end{table}
...
Table~\ref{tab:rawgrid-method-comparison} compares the three implementations for AM adjacent pairs on four margins: connected-grid share, mean connector distance, median adjacent travel time, and median directional ratio. \emph{Raw\_edgeproj\_v2} gains approximately ten percentage points of square and hex coverage over \emph{raw\_node\_v4}, with connector distances roughly one fifth shorter. In the Voronoi partition the connected-share gain is smaller, from 0.812 to 0.904, because the retained cells already sit closer to usable road access. The preferred version is \emph{raw\_edgeproj\_v2}: the coverage gain and tighter access geometry outweigh the longer adjacent travel times on the current bottleneck, which is grid entry rather than interior routing.
	...
	\subsection{Coverage of the retained edge-projection graph}

	Connector attachment and finite-pair coverage are distinct objects. Table~\ref{tab:rawgrid-method-comparison} reports the share of grids that obtain at least one connector under each raw-grid method. For the retained \emph{raw\_edgeproj\_v2} run, a stricter graph-based metric is more informative: a grid is counted as covered only if it appears in at least one adjacent pair with finite raw-network travel time. Under this criterion, square and hex count coverage remain limited, 42.5\% and 41.1\%, yet the covered cells contain 95.5\% and 95.6\% of residents. The largest weakly connected component is smaller, 34.3\% and 34.2\% of cells, but it still contains 93.7\% of residents in both systems. Voronoi remains much more connected by either margin, with 87.9\% of cells covered and 82.1\% lying in the largest weakly connected component. Table~\ref{tab:rawgrid-coverage-am} and Figure~\ref{fig:rawgrid-am-maps} summarize these AM objects.

	\begin{table}[h!]
		\centering
		\caption{AM coverage of the retained \emph{raw\_edgeproj\_v2} graph.}
		\label{tab:rawgrid-coverage-am}
		\begin{tabular}{lcccc}
			\toprule
			Grid & Covered grids & Largest comp.\ grids & Covered residents & Largest comp.\ residents \\
			\midrule
			square  & 1,427 (42.5\%) & 1,153 (34.3\%) & 95.5\% & 93.7\% \\
			hex     & 1,893 (41.1\%) & 1,575 (34.2\%) & 95.6\% & 93.7\% \\
			voronoi & 1,066 (87.9\%) &   996 (82.1\%) & 97.5\% & 95.8\% \\
			\bottomrule
		\end{tabular} \\
		\vspace{0.3em}
		\begin{minipage}{\textwidth}
			\footnotesize
			\raggedright
			Note: A grid is counted as covered when it enters at least one adjacent pair with finite raw-network travel time. ``Largest comp.'' refers to the largest weakly connected component of the undirected graph induced by those finite adjacent pairs.
		\end{minipage}
	\end{table}

	\subsection{Grid-level directional asymmetry}

	The grid-level asymmetry object differs from the centerline asymmetry measure in Section~\ref{sec:asym_sec}. There the unit is a physical corridor and the primitive is direction-specific speed. Here the unit is an undirected adjacent grid pair, and the imbalance is computed from pair-specific travel times. For each pair with finite travel time in both directions, let $ratio_{ij} = \min(t_{ij}/t_{ji},\, t_{ji}/t_{ij})$. Under the retained AM \emph{raw\_edgeproj\_v2} run, the median directional ratio is 0.793 for square grids, 0.795 for hex grids, and 0.793 for Voronoi cells. Roughly 47\% of valid pairs fall below 0.8 in all three systems, while the share below 0.5 ranges from 13.8\% in square and hex to 17.9\% in Voronoi. Table~\ref{tab:rawgrid-asymmetry-am} and Figure~\ref{fig:rawgrid-am-maps} report the corresponding AM coverage and asymmetry objects side by side. The directional signal is therefore present in the retained raw-grid object, but it remains materially weaker than the centerline benchmark reported in Table~\ref{tab:rawgrid-method-comparison}.

	\begin{table}[h!]
		\centering
		\caption{AM adjacent-pair asymmetry in the retained \emph{raw\_edgeproj\_v2} graph.}
		\label{tab:rawgrid-asymmetry-am}
		\begin{tabular}{lccccc}
			\toprule
			Grid & Both-dir pairs & Median ratio & Share $<0.8$ & Share $<0.5$ & Median gap (min) \\
			\midrule
			square  & 1,763 & 0.793 & 47.3\% & 13.8\% & 2.18 \\
			hex     & 2,574 & 0.795 & 47.0\% & 13.8\% & 1.89 \\
			voronoi & 1,658 & 0.793 & 47.4\% & 17.9\% & 2.41 \\
			\bottomrule
		\end{tabular} \\
		\vspace{0.3em}
		\begin{minipage}{\textwidth}
			\footnotesize
			\raggedright
			Note: ``Both-dir pairs'' counts undirected adjacent grid pairs with finite travel time in both directions. The directional ratio is $\min(t_{ab}/t_{ba},\, t_{ba}/t_{ab})$. The median gap is the median absolute time difference, $|t_{ab}-t_{ba}|$, in minutes.
		\end{minipage}
	\end{table}
	...
	\begin{figure}[p!]
		...
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_coverage_maps/raw_grid_coverage_square_AM.png}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_tidal_asymmetry/raw_grid_tidal_asymmetry_square_AM.png}
		\end{minipage}
		...
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_coverage_maps/raw_grid_coverage_hex_AM.png}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_tidal_asymmetry/raw_grid_tidal_asymmetry_hex_AM.png}
		\end{minipage}
		...
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_coverage_maps/raw_grid_coverage_voronoi_AM.png}
		\end{minipage}
		\hfill
		\begin{minipage}[t]{0.48\textwidth}
			\centering
			\includegraphics[width=\textwidth,height=0.21\textheight,keepaspectratio]{../../data_work/outputs/raw_connected_v1_grid_travel_time_edgeproj_v2/figures/grid_tidal_asymmetry/raw_grid_tidal_asymmetry_voronoi_AM.png}
		\end{minipage}
		\caption{AM retained \emph{raw\_edgeproj\_v2} diagnostics across the three grid systems. In each row, the left panel plots finite adjacent-pair coverage with resident circles and the largest weakly connected component outlined in red; the right panel plots the corresponding grid-level asymmetry intensity, $1-ratio_{ij}$, over adjacent pairs with finite travel time in both directions.}
		\label{fig:rawgrid-am-maps}
	\end{figure}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.8\textwidth]{../../data_work/outputs/comparison_grid_methods/fig_shared20_total_time_compare_square_AM.png}
	...
	Directional commuting costs are not a single city feature. To assess whether the same empirical object appears in a national cross-city setting, I construct a township-pair sample based on the 2021 township-level commuting flow data and query bilateral driving times through the Baidu Maps driving API. The resulting evidence is descriptive, but it establishes that asymmetric travel time is a common feature of heavily used within-city commuting links.

	The national sample is constructed in three steps:
	\begin{itemize}
		\item \textbf{City universe.} In the first round, I select cities with 50 largest commuter size, based on 2021 commuting flow data from Baidu Map.
		\item \textbf{Township-pair universe.} Within each city, same-township flows are dropped. Township coordinates are assigned from \texttt{commuting\_town\_2021\_centers.parquet}, using office coordinates when available and centroid coordinates otherwise. Rows with missing endpoint coordinates are dropped.
		\item \textbf{Pair selection.} Directed township flows are collapsed to unordered township pairs. For each unordered pair, commuter size is measured as, $(pop_{ab}+pop_{ba})/2$. The main crawling sample keeps the top 50 unordered township pairs per city by this two-way commuter measure.
	\end{itemize}

	Each retained township pair is queried in both directions. For a pair $(a,b)$, one API call records the travel time from $a$ to $b$, and the paired call records the travel time from $b$ to $a$. The API calls use \texttt{directionlite/v1/driving}. The AM and PM sessions use the congestion-avoidance routing option (\texttt{tactics=2}); the late-night baseline uses the regular fastest-time routing option (\texttt{tactics=0}) during a low-congestion clock window. The three sessions used here are:
	\begin{itemize}
		\item AM session \texttt{20260427T070801}, sampled from 07:08:01 to 08:14:32.
		\item PM session \texttt{20260427T171554}, sampled from 17:15:54 to 18:26:28.
		\item Late-night session \texttt{20260427T003434}, sampled from 00:34:35 to 01:48:51.
	\end{itemize}
	
	Two directional rows failed in the late-night session and are backfilled from an earlier complete night run, sampled from 22:22 to 23:48.

	The figures below focus on the 20 cities with the largest AM excess directional gap among the retained top-50 city sample. The ranking statistic is the commuter-weighted mean AM directional gap, measured relative to the late-night gap, across the city's top 50 township pairs.  \\ 
	
	% This restriction sharpens the descriptive exercise by concentrating on cities where peak-period directional congestion is most visible, while preserving a national cross-city design rather than a single-city case study.

	Figure~\ref{fig:national-asymmetry-hist} records the pooled distribution of the directional ratio,
	\[
		\frac{\min\{t_{ab},t_{ba}\}}{\max\{t_{ab},t_{ba}\}},
	\]
	for 1,000 township pairs, 20 cities times 50 pairs per city. A value of one means that the two directions have identical travel time within a time window. Lower values imply stronger directional asymmetry. The AM distribution lies furthest to the left: the median ratio is 0.856, and 67.5 percent of pairs have a ratio below 0.90. The PM distribution is also left of the late-night benchmark, with a median of 0.890 and 57.4 percent below 0.90. The late-night baseline is closer to symmetry, with a median ratio of 0.912. The national pattern therefore matches the motivating mechanism of the model: commute-time costs vary by direction, and the difference is most pronounced during peak travel windows.
	...
	\begin{figure}[h!]
		...
		\caption{Pooled distribution of directional asymmetry in the national top-20 congestion-city sample. The horizontal axis is the lower travel time divided by the higher travel time for the same township pair and time window. Lower values correspond to stronger directional asymmetry.}
		\label{fig:national-asymmetry-hist}
	\end{figure}

	Figure~\ref{fig:national-adjusted-asymmetry} asks whether the peak-period asymmetry exceeds the same pair's late-night directional imbalance. For each AM or PM pair, the peak-period ratio is divided by a late-night ratio aligned to the peak-period ordering of the two directions. This alignment matters because the slower direction can reverse between morning and evening commuting. Values below one indicate that the peak-period directional imbalance is stronger than the late-night imbalance for the same pair and direction ordering. The AM distribution is concentrated below one, with a median AM-to-night ratio of 0.878 and 80.1 percent of pairs more asymmetric than their late-night baseline. PM remains below one for most pairs as well, with a median PM-to-night ratio of 0.919 and 73.9 percent more asymmetric than late night. These comparisons indicate that the national asymmetry is not only a fixed feature of road geometry or signal timing; peak congestion amplifies the directional wedge.

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.88\textwidth]{../../data_work/outputs/national_tidal_commuting_top50cities_2021_v1/figures/top20_congestion_top50_note/figure3_adjusted_hist_overlay.png}
		\caption{Peak-period asymmetry relative to the late-night baseline. The denominator is the late-night directional ratio aligned to the AM or PM ordering of the two directions, so the AM and PM comparisons can use different late-night direction orderings for the same township pair. Values below one indicate stronger peak-period asymmetry than late-night asymmetry.}
		\label{fig:national-adjusted-asymmetry}
	\end{figure}

	Figure~\ref{fig:national-city-scatter} aggregates the same evidence to the city level. Each point is one city, measured by the commuter-weighted median min/max ratio across its 50 retained township pairs. The cloud is not concentrated on the 45-degree line. Some cities have stronger AM asymmetry, others have stronger PM asymmetry, and several cities have asymmetric travel times in both peaks. This cross-city dispersion is useful for the paper's empirical motivation: directional commuting costs are present across many urban systems, but their timing and intensity are heterogeneous.

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.78\textwidth]{../../data_work/outputs/national_tidal_commuting_top50cities_2021_v1/figures/top20_congestion_top50_note/figure4_city_scatter_am_pm.png}
		\caption{City-level AM and PM directional asymmetry. Each point is a city, measured by the commuter-weighted median min/max ratio across its top 50 township pairs. Points away from the 45-degree line indicate that morning and evening directional asymmetry need not have the same intensity within a city.}
		\label{fig:national-city-scatter}
	\end{figure}

	Taken together, the three figures establish a national descriptive fact that motivates the model. In the most congested cities among the national top-50 commuting markets, heavily used township pairs frequently have direction-specific travel times, and this wedge widens during commuting peaks relative to late night. The evidence does not identify the welfare value of tidal-lane reallocation by itself. It does, however, support the paper's maintained measurement object: bilateral commuting costs should be allowed to differ by direction when peak-period road capacity is scarce.
