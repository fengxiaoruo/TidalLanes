...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.6\textwidth]{../00archive/exhibit/map_raw_roads_15km.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.6\textwidth]{../00archive/exhibit/map_buffer_surface_15km.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.6\textwidth]{../00archive/exhibit/map_centerline_15km.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.6\textwidth]{../00archive/exhibit/fig_length_cdf_4networks.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{../00archive/exhibit/map_raw_split_matched_vs_unmatched.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{../00archive/exhibit/map_undirected_centerline_coverage_4classes.png}
	...
	Figure~\ref{fig:match-quality-visual} provides visual diagnostics of the matching
	quality, including distributions of coverage ratios, length differences, and
	scatter plots comparing centerline lengths to matched segment lengths. These
	diagnostics confirm that the majority of matches exhibit reasonable geometric
	consistency, with coverage ratios clustered near 1.0 and length relationships
	following an approximately linear pattern.

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.95\textwidth]{../00archive/interim_data/reports/match_quality_centerline.png}
		\caption{Visual diagnostics of matching quality: (top left) distribution of
			coverage ratios, (top right) distribution of length differences, (bottom left)
			scatter plot of centerline length vs.\ matched length, (bottom right) coverage
			ratio vs.\ centerline length.}
		\label{fig:match-quality-visual}
	\end{figure}
	\end{comment}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.7\textwidth]{../00archive/interim_data/asym/hist_speed_raw_vs_centerline_all.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.95\textwidth]{../00archive/interim_data/asym/diurnal_raw_centerline_combined.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.95\textwidth]{../00archive/interim_data/asym/hist_speed_ratio_am_pm.png}
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.85\textwidth]{../00archive/interim_data/asym/figs_asym/map_tidal_candidates_150km.png}
	...
	\begin{figure}[h!]
		...
		\begin{minipage}[t]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../00archive/interim_data/asym/figs_asym/map_asym_AM_unsym1_150km.png}
		...
		\begin{minipage}[t]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{../00archive/interim_data/asym/figs_asym/map_asym_PM_unsym1_150km.png}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
	...
	\begin{itemize}
		\item \textbf{Link length} $\text{dist}_{kl}$: length-weighted average over matched centerline segments.
		\item \textbf{Observed (congestion-inclusive) speed} $v_{kl}^{\text{obs}}$: time-weighted harmonic mean peak-period speed from matched centerline observations.
	...
	\end{equation}
	measured in minutes and reflecting equilibrium congestion. The free-flow travel time $\bar{t}_{kl}=\ell_{kl}/v^0_{kl}$ (equation \eqref{eq:tbar}) is not directly observed; the free-flow speed $v^0_{kl}$ is treated as a link-direction fixed effect and recovered jointly with the congestion elasticity $\lambda$ in the regression \eqref{eq:est_lambda}. Directionality is preserved throughout: $t_{kl}$, $t_{lk}$, $n_{kl}$, $n_{lk}$ all potentially differ, directly capturing the tidal asymmetry in road infrastructure.
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.95\textwidth]{../00archive/interim_data/figs/grid_links_square3km_AM_two_panels.png}
	...
	\paragraph{Readability Filter} Because not all node pairs $(i,j)$ with commuting flow data are mutually reachable. Prior to computing commuting costs, we restrict attention to the largest weakly connected component (i.e., drop OD pairs with home and work nodes that are not connected in the centerline road network\footnote{ In the square 3km grid system, the raw commuting data contain 443,207 distinct grid--grid origin--destination pairs, covering a total commuting population of 7.96 million individuals. After restricting attention to grid pairs whose home and work locations lie in the same weakly connected component of the directed road network, 355,649 OD pairs remain (80.2\% of all pairs), accounting for 7.54 million commuters (94.6\% of the total population). This indicates that this step primarily removes low-volume and peripheral commuting pairs, while preserving the vast majority of economically relevant commuting flows. }). This step ensures that the route aggregator and the implied bilateral commuting costs are well-defined on the analysis sample and avoids numerical artifacts driven by disconnected nodes.
	...
	These values lie within the range commonly used in the urban economics literature and imply economically meaningful but moderate agglomeration and amenity externalities. This choice of parameter values guarantees the existence and uniqueness of the spatial equilibrium.

	\paragraph{Congestion elasticity $\lambda$.}
	The congestion elasticity $\lambda\geq 0$ governs how strongly realized travel time responds to traffic per lane, and thus determines the magnitude of the travel-time improvement from adding a lane. From the congestion technology \eqref{eq:tkl_expand}, combined with $t_{kl}=\ell_{kl}/v^{\text{obs}}_{kl}$, the observed speed satisfies
	\begin{equation}
		v^{\text{obs}}_{kl}
		=
		v^0_{kl}
		...
		\right)^{-\lambda}.
	\end{equation}
	Taking logs yields the estimating equation:
	\begin{equation}\label{eq:est_lambda}
		\ln v^{\text{obs}}_{kl}
		\;=\;
		\underbrace{\ln v^0_{kl}}_{\mu_{kl}}
		\;-\;
		\lambda\,\ln\!\left(\frac{\phi_{kl}}{n_{kl}}\right)
		\;+\; \varepsilon_{kl},
	\end{equation}
	that is, observed speed decreases with traffic density (traffic per lane) with elasticity $\lambda$. The link-specific free-flow speed $v^0_{kl}$ is absorbed as a link fixed effect $\mu_{kl}$.

	Ordinary least squares is inconsistent because links with high free-flow speed attract more traffic, inducing a positive correlation between $\phi_{kl}/n_{kl}$ and the residual. We therefore instrument for $\phi_{kl}/n_{kl}$ using observables that shift traffic demand but are orthogonal to link-specific free-flow conditions, following the identification strategy of Allen \& Arkolakis (2022, Section~5.3).
	...
	\begin{comment}
		Equations \eqref{eq:invert_R}--\eqref{eq:invert_F} define a fixed point in the vectors $\{\bar u_i^\theta\}_{i\in\mathcal N}$ and $\{\bar A_i^\theta\}_{i\in\mathcal N}$, which we solve numerically under a normalization (e.g.\ $\frac{1}{N}\sum_i\log\bar u_i=0$ and $\frac{1}{N}\sum_i\log\bar A_i=0$, or an equivalent level normalization). The resulting inverted fundamentals summarize residual amenity and productivity components after accounting for endogenous spillovers and commuting costs.
	\end{comment}
	...
	\begin{table}[htbp]\centering
		...
		\begin{tabular}{lll}
			...
			& $t_{kl}$ 
			& Segment speeds aggregated to grid links \\
			...
			OD commuting cost 
			& $\tau_{ij}$ 
			& Optimal routing (log-sum over paths) \\
	...
	\begin{figure}[h!]
		\centering
		\includegraphics[width=1\textwidth]{../00archive/interim_data/figs/asym_ratio_hist_2panel_square3km.png}
		\caption{Distribution of directional speed asymmetry at the grid level.
			The figure reports the distribution of speed ratios
			$\min(v_{ab}/v_{ba},\, v_{ba}/v_{ab})$ for grid-to-grid links during AM and PM peaks.
			Vertical dashed lines indicate asymmetry thresholds used in the analysis.}
...
\bibliography{tidal_lanes_refs}




\section{Incorporate asymmetric links to grid-level model}
\subsection{Method 1: Speed-Based Inference}
\begin{enumerate}
	\item Step 0: Grid coverage test. If a segment lies entirely within one grid $\Rightarrow$ only contributes to per-grid stock (lane-km, speed). If it crosses multiple grids $\Rightarrow$ continue.
	\item Step 1: Split by grid boundaries $\Rightarrow$ sub-segments, each assigned to one grid.
	\item Step 2: Order sub-segments along the original segment $\Rightarrow$ grid sequence $A \to B \to C \to \dots$.
	\item Step 3: Create directed links from consecutive grids, e.g., $A \to B, B \to C$. Attributes: length, lane-km, speed (averaged), flow (via BPR or speed–flow function).
	\item Step 4: Aggregate across all segments $\Rightarrow$ grid-level directed network $(C_{ij}, v_{ij}, q_{ij})$.
\end{enumerate}

\subsection{Method 2: Commuting Flow-Based Inference}
\begin{enumerate}
	\item Assume AM peak (e.g. 7–9am). Distribute commuting flows into 10-minute intervals using departure time distributions.
	\item Use travel survey information to infer mode shares; extract road-related flows.
	\item Construct grid-to-grid flows for each time interval.
	\item Assign OD flows to road network links using shortest paths (Dijkstra or Fast Marching).
\end{enumerate}

\subsection{Comparison and Validation}
\begin{itemize}
	\item Cross-validate speed-based and commuting-based measures.
	\item Identify robust asymmetric patterns (AM vs. PM flows).
	\item Problems relating to the flow infer.
\end{itemize}
